...
@bot.message_handler(func=lambda message: message.text in ["Получить бинго","Принять участие в бинго","Войти в админку"])
def handle_button(message):
    ...
    if message.text == "Войти в админку" and message.from_user.id in ids:
...
def user_keyboard(user_id):
    ...
    return markup
#for admin
@bot.message_handler(func=lambda message: message.text in ["Добавить пункт","Узнать все пункты", "Удалить пункт","Выйти из админки"])
def handle_button(message):
